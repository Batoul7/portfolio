...
const Modal: React.FC<ModalProps> = ({ isOpen, onClose, title, description, items,link }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-my-primary bg-opacity-50 backdrop-blur-sm px-5">
            <div data-aos="zoom-in" className="bg-white dark:bg-my-primary rounded-2xl p-8 w-full max-w-2xl  shadow-inner shadow-my-secondery">
                <div className="mb-6 flex flex-col gap-5">
                    ...
                    <ul className="flex flex-col gap-3">
                        {items.map((item, index) => (
                            <li key={index} className='flex gap-3 items-center'><FontAwesomeIcon icon={faCheck} className='text-my-secondery'/>{item}</li>
                        ))}
                    </ul>
                </div>
                ...
                <button
                    onClick={onClose}
                    className="block px-4 py-2 text-lg ml-auto w-fit shadow-btnshadow rounded-lg md:text-lg font-medium font-poppins hover:bg-my-secondery transition-all" >
                    OK
                </button>
                </div>
