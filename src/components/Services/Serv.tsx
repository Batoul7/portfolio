...
export default function Serv({data} : ServComponentProps) {
  ...
  return (
    <>
    {data.map((serv,index) => (
        <div  data-aos="fade-up" data-aos-duration="2000" key={index} className="relative border border-opacity-95 border-my-secondary rounded-lg
        flex flex-col gap-5 overflow-hidden group">
            ...
            <div className="absolute flex flex-col gap-2 bg-black bg-opacity-70 w-full h-full justify-center items-center px-3  text-center">
                ...
                <h4 className="text-lg sm:text-lg md:text-2xl font-semibold text-my-secondary">{serv.title}</h4>
                <p className="text-sm  text-white">{serv.desc}</p>
                <button onClick={() => handleOpenModal(serv)} className="w-fit text-base md:text-lg font-semibold text-my-secondary">
